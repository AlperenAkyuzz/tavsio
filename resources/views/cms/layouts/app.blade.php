        ...
        <div class="d-flex align-items-center mt-5">
            ...
            <div class="d-flex flex-column">
                ...
                <div class="navi mt-2">
                    ...
                    <a href="{{url('tavsiocms/logout')}}" class="btn btn-sm btn-light-danger font-weight-bolder py-2 px-5">Sign Out</a>
                </div>
            </div>
        </div>
        <!--end::Header-->
...
<script type="text/javascript">
    if(typeof dropify != 'undefined' && dropify == true){
        $('.dropify').dropify({
            messages: {
                'default': 'Resim buraya sürükleyin veya tıklayıp yükleyin.',
                'replace': 'Resim değiştirmek için buraya sürükleyin veya tıklayıp yükleyin.',
                'remove':  'Sil',
                'error':   'Hata, lütfen tekrar deneyin!'
            }
        });
    }
    if(typeof ozeldersDatatable != 'undefined' && ozeldersDatatable == true){
        if(typeof ozeldersData == 'undefined'){
            var ozeldersData = {};
        }
...
*/
    ...
    function changeData(checkbox,uuid,url){
        let status = 1;
        if(!checkbox.checked){
            status = 0;
        }
    ...
    @if(session('status'))
        @if(session('status') == 'success')
            responseMessage('{{session('status')}}','{{session('responseMessage')}}');
        @else
            responseMessage('{{session('status')}}','{{session('responseMessage')}}');
        @endif
    @endif
